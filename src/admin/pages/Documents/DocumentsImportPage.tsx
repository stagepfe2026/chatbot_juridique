...
export default function DocumentsImportPage() {
  ...
  async function onSubmit() {
    ...
    if (!form.file) return setSubmitError("Veuillez sélectionner un fichier.");
    if (fileError) return setSubmitError(fileError);
    if (!form.title.trim()) return setSubmitError("Le titre est obligatoire.");
    if (!form.category) return setSubmitError("La catégorie est obligatoire.");

    try {
      setIsSubmitting(true);
      const res = await importDocument(form);
      setSuccessMsg(`Document importé: ${res.filename} (statut: ${res.status}).`);
      setForm(initialForm);
      setFileError(null);
    } catch (e: unknown) {
      const message =
    ...
    <div className="page">
      ...
      <section className="card">
        ...
        >
          ...
          <div style={{ marginTop: 12, fontSize: 13, opacity: 0.8 }}>Formats acceptés : PDF, DOCX</div>
          {form.file && (
