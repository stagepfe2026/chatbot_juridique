...
export default function MyClaimsPage() {
  const { t, claimCategoryLabel, claimPriorityLabel, claimStatusLabel } =
    useI18n();
  ...
  function onResolutionFeedback(claimId: string, feedback: ResolutionFeedback) {
    ...
    publishSnackbar({
      ...
      message:
        feedback === "CONFIRMED"
          ? t("claims.resolutionClosed")
          : t("claims.resolutionContinues"),
    });
  }
  ...
  const enrichedClaims = useMemo<EnrichedClaim[]>(() => {
    return claims
      .map((claim, index) =>
        enrichClaim(claim, index, resolutionFeedback[claim.id]),
      )
      .sort(
        (a, b) =>
  ...
  const stats = useMemo(() => {
    return {
      ...
      open: filteredClaims.filter((claim) =>
        ["SUBMITTED", "UNDER_REVIEW", "PROCESSING"].includes(
          claim.displayStatus,
        ),
      ).length,
      resolved: filteredClaims.filter(
        (claim) => claim.displayStatus === "RESOLVED",
      ).length,
      closed: filteredClaims.filter((claim) => claim.displayStatus === "CLOSED")
        .length,
    };
  }, [filteredClaims]);
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-lg">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
          ...
          <select
            value={status}
            onChange={(event) =>
              setStatus(event.target.value as DetailedClaimStatus | "")
            }
          ...
          <select
            value={category}
            onChange={(event) =>
              setCategory(event.target.value as ClaimCategory | "")
            }
            className="h-11 rounded-xl border border-slate-200 px-3 text-sm outline-none transition focus:border-red-300 focus:ring-4 focus:ring-red-50"
          >
            <option value="">{t("common.allCategories")}</option>
            {(
              ["ACCOUNT", "CHATBOT", "DOCUMENT", "OTHER"] as ClaimCategory[]
            ).map((value) => (
          ...
          <select
            value={priority}
            onChange={(event) =>
              setPriority(event.target.value as ClaimPriority | "")
            }
            className="h-11 rounded-xl border border-slate-200 px-3 text-sm outline-none transition focus:border-red-300 focus:ring-4 focus:ring-red-50"
          >
            <option value="">{t("common.allPriorities")}</option>
            {(["LOW", "NORMAL", "HIGH", "URGENT"] as ClaimPriority[]).map(
              (value) => (
                <option key={value} value={value}>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-4">
          <StatCard
            label={t("claims.ticketsShown")}
            value={String(stats.total)}
            helper={t("claims.afterFilters")}
          />
          <StatCard
            label={t("claims.open")}
            value={String(stats.open)}
            helper={t("claims.openHelper")}
          />
          <StatCard
            label={t("claims.resolved")}
            value={String(stats.resolved)}
            helper={t("claims.resolvedHelper")}
          />
          <StatCard
            label={t("claims.closed")}
            value={String(stats.closed)}
            helper={t("claims.closedHelper")}
          />
        </div>
        ...
        <div className="mt-5 grid gap-4">
          ...
          {!loading &&
            filteredClaims.map((claim) => (
              <article
                key={claim.id}
                className="rounded-3xl border border-slate-200 bg-white p-5 transition hover:border-red-200 hover:shadow-lg"
              >
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-2">
                      <span
                        className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${statusBadgeClass(claim.displayStatus)}`}
                      >
                        {claimStatusLabel(claim.displayStatus)}
                      </span>
                      <span
                        className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${priorityBadgeClass(claim.priority ?? "NORMAL")}`}
                      >
                        {claimPriorityLabel(claim.priority ?? "NORMAL")}
                      </span>
                      <span className="inline-flex rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-600">
                        {claim.messageTotal} {t("claims.exchanges")}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid gap-2 text-sm text-slate-500 lg:text-right">
                    <div>
                      {t("claims.lastUpdated")}:{" "}
                      {formatClaimDate(claim.updatedAt)}
                    </div>
                    <div>
                      {claim.assignedAgent?.name
                        ? `${t("claims.assignedTo")} ${claim.assignedAgent.name}`
                        : t("claims.pendingAssignment")}
                    </div>
                    <div>
                      {claim.dueAt
                        ? `${t("claims.dueDate")}: ${formatClaimDate(claim.dueAt)}`
                        : t("claims.dueCalculating")}
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-4 grid gap-3 xl:grid-cols-[minmax(0,1fr)_320px]">
                  <div className="rounded-2xl bg-slate-50 px-4 py-4 text-sm text-slate-600">
                    <div className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">
                      {t("claims.latestReply")}
                    </div>
                    <p className="mt-2 leading-6">{claim.summaryReply}</p>
                  </div>
                  <div className="rounded-2xl border border-slate-200 bg-white px-4 py-4">
                    <div className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">
                      {t("claims.actionHistory")}
                    </div>
                    <div className="mt-3 grid gap-3">
                      {claim.history.slice(0, 4).map((entry) => (
                        <div
                          key={entry.id}
                          className="relative pl-5 text-sm text-slate-600 before:absolute before:left-0 before:top-1.5 before:h-2 before:w-2 before:rounded-full before:bg-red-500"
                        >
                          ...
                          <div>
                            {entry.actorName} |{" "}
                            {formatClaimDate(entry.createdAt)}
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {claim.displayStatus === "RESOLVED" ? (
                  <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-4">
                    <div className="text-sm font-semibold text-emerald-800">
                      {t("claims.confirmResolution")}
                    </div>
                    <p className="mt-1 text-sm text-emerald-700">
                      {t("claims.resolutionText")}
                    </p>
                    <div className="mt-3 flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={() =>
                          onResolutionFeedback(claim.id, "CONFIRMED")
                        }
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          onResolutionFeedback(claim.id, "REOPENED")
                        }
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <Link
                    ...
                    className="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-red-200 hover:text-red-600"
                  >
                    {t("common.viewRecord")}
                  </Link>
                  {claim.pageContext ? (
                    <a
                      href={claim.pageContext}
                      className="inline-flex items-center justify-center rounded-xl bg-red-600 px-4 py-2 text-sm font-semibold text-white no-underline transition hover:bg-red-700"
                    >
      ...
      <aside className="grid gap-4">
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-lg">
          ...
          <div className="mt-4 grid gap-3 text-sm text-slate-600">
            <WorkflowRow
              label={claimStatusLabel("SUBMITTED")}
              helper={t("claims.workflowSubmitted")}
            />
            <WorkflowRow
              label={claimStatusLabel("UNDER_REVIEW")}
              helper={t("claims.workflowReview")}
            />
            <WorkflowRow
              label={claimStatusLabel("PROCESSING")}
              helper={t("claims.workflowProcessing")}
            />
            <WorkflowRow
              label={claimStatusLabel("RESOLVED")}
              helper={t("claims.workflowResolved")}
            />
            <WorkflowRow
              label={claimStatusLabel("CLOSED")}
              helper={t("claims.workflowClosed")}
            />
          </div>
        </section>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-lg">
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">
            {t("claims.recentActions")}
          </p>
          <div className="mt-4 grid gap-4">
            {recentActions.length === 0 ? (
              <p className="text-sm text-slate-500">
                {t("claims.recentHistoryEmpty")}
              </p>
            ) : null}
            {recentActions.map((entry) => (
              <div
                key={entry.id}
                className="relative pl-5 text-sm text-slate-600 before:absolute before:left-0 before:top-1 before:h-2.5 before:w-2.5 before:rounded-full before:bg-red-500"
              >
                <div className="font-semibold text-slate-800">
                  {entry.description}
                </div>
                ...
                <div>
                  {entry.subject} | {formatClaimDate(entry.createdAt)}
                </div>
              </div>
            ))}
          </div>
  ...
  return {
    ...
    assignedAgent:
      ...
      (displayStatus !== "SUBMITTED"
        ? { id: `agent-${claim.id}`, name: "Service reclamations" }
        : null),
    dueAt,
    displayStatus,
    ticketNumber,
    ...
    summaryReply:
      claim.lastAdminReplyPreview ??
      claim.adminReply ??
      "Aucune reponse administrative pour le moment.",
    history,
  };
}
  ...
  if (["RESOLVED", "CLOSED"].includes(displayStatus))
    entries.push({
      ...
      description: claim.adminReply
        ? "Reponse administrative transmise"
        : "Resolution communiquee",
